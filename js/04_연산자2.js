...
function check1(){
  ...
  // 동등 비교(숫자 == 문자열) 확인
  console.log(`${num1} == "${str2}" => ${num1 == num2}`); 
...
function oddEvenCheck(){
  ...
  // Number()를 이용해 숫자로 변한 후 value 변수에 대입(저장)
...
function multipleCheck(){
  ...
  // 배수 확인에 필요한 요소를 얻어와 변수에 저장
   ...
   const num2 = Number(input2.value); // 입력한 값 숫자로 변환
   const num3 = Number(input3.value); // 입력한 값 숫자로 변환



  // 10은  5 배수?  -> 10을 5로 나눴을 때 나머지가 0인지 확인
...
function orCheck(){
  ...
  console.log(`4는 10를 초과하거나 짝수인가? ${bool1}`);
