...
addStudent.addEventListener("click", () => {
  ...
  const std = new Student(inputGrade.value, inputBan.value, 
                          inputNumber.value, inputName.value);
...
// 학생 목록 출력하기
const printSrudent = document.querySelector("#printStudent");
const result = document.querySelector("#result");

// 현재 학생 목록 출력 버튼 클릭 시 
printSrudent.addEventListener("click", () => {

 /* 이전 #result에 작성되어있는 내용 삭제  */
 result.innerHTML = '';

  // srudentList에 존재하는 모든 학생 객체 접근하기
...
checkJSON.addEventListener("click", () => {
  ...
  console.log('json2 확인', json2, typeof json2); // string
