...
function letCheck(){
  ...
  // 새로운 변수 선언
  let name2 = "신짱구";
  console.log("name : ", name, "neme2 : ", name2);
}
...
function constCheck(){
  ...
  // 2_변수와자료형.js:103 -> 103번째 줄에서 오류 발생 
...
function scopeTest(){
  ...
  function scopeTest2(){
    ...
    console.log("if 실핸 전 fn2 : ", fn2); // 함수 지역 변수2
    ...
    if(1 == 1){ // 1과 1이 같다면 if {} 수행
      ...
      console.log("if 내부에서 global1 : ", global1); // 출력 0

      // 넓은 범위(함수)에서 선언된 변수 -> 좁은 범위(함수 내 if)에서 사용 가능
      console.log("if 내부에서 fn1: ", fn1); // 출력 0
      console.log("if 내부에서 local1 : ", local1); // 출력 0
    ...
    // if문 지역 변수 local1을 if문 밖에서 호출하면 "정의 되지 않음" 오류 발생
  ...
  function typeCheck(){
    ...
    console.log("flase : ", false, typeof false);
